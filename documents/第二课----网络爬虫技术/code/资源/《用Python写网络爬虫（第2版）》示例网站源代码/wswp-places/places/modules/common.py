...
class Places:
    def __init__(self, db, expire_db_secs=3600):
        ...
        db.define_table('places', 
            ...
            Field('country_or_district_id', 'integer', writable=False, readable=False),
            Field('flag_img', label='Flag', writable=False, represent=lambda path: IMG(_src=path)),
            ...
            Field('country_or_district', label='Country (District)', writable=False),
    ...
    def get(self, country_or_district_id=None, iso=None):
        if country_or_district_id is not None:
            logic = self.db.places.country_or_district_id == country_or_district_id
    ...
    def search(self, logic=None, limitby=None):
        return self.db(logic).select(limitby=limitby, orderby=self.db.places.country_or_district_id)
...
AF,Afghanistan,Kabul,647500,29121286,AS,.af,AFN,Afghani,93,,,"fa-AF,ps,uz-AF,tk","TM,IR,TJ,PK,UZ"
...
BD,Bangladesh,Dhaka,144000,156118464,AS,.bd,BDT,Taka,880,####,^(\d{4})$,"bn-BD,en","MM"
...
BT,Bhutan,Thimphu,47000,699847,AS,.bt,BTN,Ngultrum,975,,,dz,
...
KG,Kyrgyzstan,Bishkek,198500,5508626,AS,.kg,KGS,Som,996,######,^(\d{6})$,"ky,uz,ru","TJ,UZ,KZ"
...
KP,North Korea,Pyongyang,120540,22912177,AS,.kp,KPW,Won,850,###-###,^(\d{6})$,ko-KP,"KR,RU"
...
KZ,Kazakhstan,Astana,2717300,15340000,AS,.kz,KZT,Tenge,7,######,^(\d{6})$,"kk,ru","TM,KG,UZ,RU"
LA,Laos,Vientiane,236800,6368162,AS,.la,LAK,Kip,856,#####,^(\d{5})$,"lo,fr,en","MM,KH,TH,VN"
...
MM,Myanmar,Nay Pyi Taw,678500,53414374,AS,.mm,MMK,Kyat,95,#####,^(\d{5})$,my,"LA,TH,BD"
MN,Mongolia,Ulan Bator,1565000,3086918,AS,.mn,MNT,Tugrik,976,######,^(\d{6})$,"mn,ru","RU"
...
NP,Nepal,Kathmandu,140800,28951852,AS,.np,NPR,Rupee,977,#####,^(\d{5})$,"ne,en",
...
PK,Pakistan,Islamabad,803940,184404791,AS,.pk,PKR,Rupee,92,#####,^(\d{5})$,"ur-PK,en-PK,pa,sd,ps,brh","AF,IR"
...
RU,Russia,Moscow,17100000,140702000,EU,.ru,RUB,Ruble,7,######,^(\d{6})$,"ru,tt,xal,cau,ady,kv,ce,tyv,cv,udm,tut,mns,bua,myv,mdf,chm,ba,inh,tut,kbd,krc,ava,sah,nog","GE,BY,UA,KZ,LV,PL,EE,LT,FI,MN,NO,AZ,KP"
...
TJ,Tajikistan,Dushanbe,143100,7487489,AS,.tj,TJS,Somoni,992,######,^(\d{6})$,"tg,ru","AF,KG,UZ"
...
VN,Vietnam,Hanoi,329560,89571130,AS,.vn,VND,Dong,84,######,^(\d{6})$,"vi,en,fr,zh,km","LA,KH"
...
AN,Netherlands Antilles,Willemstad,960,136197,NA,.an,ANG,Guilder,599,,,"nl-AN,en,es",GP"""
        ...
        # sort by country or district name
        ...
        for i, row in enumerate(rows):
            ...
            for field in self.numeric_fields:
                ...
                record[field] = value
            # add country_or_district_id for ordering on GAE, which does not use sequential ID's
            record['country_or_district_id'] = i + 1
            record['flag_img'] = URL(c='static', f='images/flags/%s.png' % record['iso'].lower())
